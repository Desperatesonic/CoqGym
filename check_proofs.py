...
def check_file(filename, sexp_cache, args):
    ...
    with SerAPI(args.timeout, args.debug) as serapi:
        ...
        for num_executed, (code_line, tags) in enumerate(coq_code):
            ...
            file_data['vernac_cmds'].append((code_line, tags['VERNAC_TYPE'], sexp_cache.dump(ast)))
        
    return file_data
   

def dump(file_data, args):
    path = os.path.join(args.data_path, file_data['coq_project'], file_data['filename'])[:-2]
    try:
...
if __name__ == '__main__':
    ...
    import argparse
    arg_parser = argparse.ArgumentParser(description='Execute the Coq source code and locate the proofs')
    arg_parser.add_argument('--debug', action='store_true', help='print the executed Coq code')
    arg_parser.add_argument('--proj', type=str, help='The Coq project to process (default to all projects)')
